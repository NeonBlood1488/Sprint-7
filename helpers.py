...
def generate_random_string(length=10):    # Генерация случайной строки для логина/пароля/имени
    letters = string.ascii_lowercase
    return ''.join(random.choice(letters) for _ in range(length))

def create_courier(login=None, password=None, first_name=None):    # Создание курьера
...
def login_courier(login, password):                                # Логин курьера
...
def delete_courier(courier_id):          # Удаление курьера по id
